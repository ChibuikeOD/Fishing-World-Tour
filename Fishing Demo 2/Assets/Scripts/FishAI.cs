...
public class FishAI : MonoBehaviour
{
    ...
    //Set the fish info when it is created
    //Fish Info
    ...
    void Bite()
    {
        ...
        if (waiting == false)
        {
            //Mark the bobber as occupied so other fish stop interacting with it
            biting = true;

            //Tell bobber this fishes data
            Debug.Log("Setting " + fishInfo + " to bobber");
            bobber.GetComponent<Bobber>().hookedFish = fishInfo;

            StartCoroutine(BiteWait());
        }

        if (bobber.GetComponent<Bobber>().occupied == true || bobber == null)
        {
            state = State.Fleeing;
        }

        //If the bobber is hooking, pull away on the line
        else if (bobber.GetComponent<Bobber>().hooking == true)
        {
            bobber.GetComponent<Bobber>().occupied = true;
